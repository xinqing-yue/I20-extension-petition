...
\begin{theorem}1
\textit{Under continuation ratio model, with the OBD decision criterion given in (1) and $\lambda =0$, with the restriction in (2) . When $\beta_1=\beta_2$,
\begin{enumerate}
    \item  If $\alpha_2 < -2 log\left(\frac{\pi^*}{1-\pi^*}\right),    \frac{log(\frac{\pi^*}{1-\pi^*})-\alpha_1}{\beta_1}$ is the OBD. 
    \item If $\alpha_2 \geq -2log\left(\frac{\pi^*}{1-\pi^*}\right)$ and $\alpha_1 + \alpha_2 \le 0, \frac{-\alpha_1-\alpha_2}{2\beta_1}$ is the OBD.
\end{enumerate}
}
\end{theorem}
\textit{\begin{theorem}2
Under continuation ratio model, with OBD decision criterion given in (1) with $\lambda=0$, with the restriction in (2). When $\beta_1 \neq \beta_2$, with the maximum tolerable toxicity rate  $\pi^* <0.5$,
\begin{enumerate}
    \item when $\beta_1(exp(\alpha_2-\frac{\alpha_1\beta_2}{\beta_1})+1)\le 2\beta_2, \frac{log(\frac{\pi^*}{1-\pi^*})-\alpha_1}{\beta_1}$ is the OBD; 
    \item when $\beta_1(exp(\alpha_2-\frac{\alpha_1\beta_2}{\beta_1})+1)> 2\beta_2$,and $ \beta_1(1+exp(\alpha_2))< \beta_2(1+exp(-\alpha_1))$, OBD exists but has no close form.
\end{enumerate}
\end{theorem}}
\textit{\begin{theorem}3
Under continuation ratio model, with $\beta_1 \neq \beta_2$, and $ \beta_1(exp(\alpha_2-\frac{\alpha_1\beta_2}{\beta_1})+1)\le 2\beta_2$, $\forall \pi^{*} < 0.5$ (parameters restriction according to Theorem 2),  the C-optimal design for estimating OBD is single point design, and the point is $\frac{log(\frac{\pi^*}{1-\pi^*})-\alpha_1}{\beta_1}$.
\end{theorem} 
}
\subsection{OBDs Identification and Optimal Designs when $\lambda \in [0,1]$}
\textit{OBD Identification When $\beta_1 =\beta_2$}
\textit{
\begin{theorem}4 Under continuation ratio model, with the OBD decision criterion given in (1), when $\lambda$, as an adjustable constant in $[0,1]$ , with the restriction in (2). Two parameter restrictions are:
\begin{itemize}
    \item 
    $0 \le \lambda < \min(1,\frac{e^{\alpha_2}}{e^{\alpha_1}-e^{\alpha_2}})$ (If $ \frac{e^{\alpha_2}}{e^{\alpha_1}-e^{\alpha_2}} >1$, $\lambda$ can equal $1$) and  $\alpha_2< \alpha_1 < log(\pi^*/(1-\pi^*))$;
    \item $\alpha_1<log(\pi^*/(1-\pi^*))$ and $\alpha_2 >\alpha_1$.
\end{itemize}
Let $root= \frac{-e^{\alpha_1}e^{\alpha_2} \lambda + \sqrt{-e^{\alpha_1}e^{2\alpha_2}(e^{\alpha_1}\lambda -e^{\alpha_2}(1+\lambda))}}{e^{\alpha_1}e^{2\alpha_2}(1+\lambda)}$, when $\beta_1=\beta_2$,
\begin{enumerate}
    \item  If either of the two parameter restrictions satisfied, when $root \geq \frac{\pi^*/(1-\pi^*)}{e^{\alpha_1}} $,  dose $\frac{log(\frac{\pi^*}{1-\pi^*})-\alpha_1}{\beta_1}$ is the OBD.
    \item  If either the two parameter restriction mentioned above satisfied, when $root \in (1,\frac{\pi^*/(1-\pi^*)}{e^{\alpha_1}})$,  dose $\frac{log(root)}{\beta_1}$ is the OBD.
\end{enumerate}
\end{theorem}}

\section{My current work}
Currently, I research the performing of simulation in order to do comparisons between my algorithm and some relevant algorithms now. I will compare it with the algorithm proposed in Zhang et al.[14], standard CRM, and other CRM based algorithms.\\
I'm also working on the theoretical part where $\lambda$ from the decision criterion takes value in $[0,1]$. It consists of two parts: $\beta_1 = \beta_2$;  $\beta_1 \neq \beta_2$. I have finished the part that $\beta_1$ equals $\beta_2$.  When $\beta_1 \neq \beta_2$, OBDs have no close form, so it is hardly to identify them. I decide to discuss them with typical examples based on the shape of dose-response curves. 
